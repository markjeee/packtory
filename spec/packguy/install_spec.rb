...
describe 'Install of generated package' do
  context 'Fpm debian package' do
    before do
      ...
      @pkg_file_path = PackguySpec::DownloadGems.find_or_compile_package('fpm')
    ...
    it 'should install in Xenial' do
      ...
      container = DockerTask.containers['packguy-spec.xenial']

      container.shhh do
        container.pull
        container.runi(:exec => '"/build/spec/exec/install_fpm_deb %s"' % PackguySpec.calculate_build_path(@pkg_file_path))
      end
    ...
    it 'should install in Bionic' do
      FileUtils.rm_f(@pkgout_file_path) if File.exists?(@pkgout_file_path)
      container = DockerTask.containers['packguy-spec.bionic']
    ...
    it 'should install in Jessie' do
      FileUtils.rm_f(@pkgout_file_path) if File.exists?(@pkgout_file_path)
      container = DockerTask.containers['packguy-spec.jessie']
    ...
    it 'should install in Stretch' do
      FileUtils.rm_f(@pkgout_file_path) if File.exists?(@pkgout_file_path)
      container = DockerTask.containers['packguy-spec.stretch']
